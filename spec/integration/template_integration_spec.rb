...
RSpec.describe "template.rb integration", skip: (ENV["RUN_INTEGRATION"] != "1" ? "Set RUN_INTEGRATION=1 to run" : false) do
  ...
  describe "Hotwire minimal (importmaps, RSpec, sqlite)" do
    # use_defaults? n | react? n | importmaps? y | shadcn? n | lograge? n |
    # sidekiq? n | hashdiff? n | auth0? n | rspec? y | postgres? n |
    # ruby_native? n | standard_ruby? n | overmind? n | commit? n
    let(:app_name) { "hotwire_minimal" }
    let(:answers) { %w[n n y n n n n n y n n n n n] }
  ...
  describe "React minimal (Inertia/Vite, sqlite)" do
    ...
    # auth0? n | rspec? n | postgres? n | ruby_native? n | standard_ruby? n |
    # overmind? n | commit? n   (no importmaps prompt for React)
    let(:app_name) { "react_minimal" }
    let(:answers) { %w[n y n n n n n n n n n n] }
  ...
  describe "Hotwire full (PG + Sidekiq + Auth0 + Guest + RSpec)" do
    ...
    # ruby_native? n | standard_ruby? n | overmind? n | <cid> | <secret> | <domain> | guest? y |
    # commit? n
    let(:app_name) { "hotwire_full" }
    let(:answers) { %w[n n y n y y y y y y n n n test-cid test-csec test.auth0.com y n] }
  ...
  describe "React + Postgres + RSpec" do
    ...
    # use_defaults? n | react? y | lograge? n | sidekiq? n | hashdiff? n |
    # auth0? n | rspec? y | postgres? y | ruby_native? n | standard_ruby? n |
    # overmind? n | commit? n
    let(:app_name) { "react_pg_rspec" }
    let(:answers) { %w[n y n n n n y y n n n n] }
